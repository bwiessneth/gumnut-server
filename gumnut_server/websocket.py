...
def update():
	...
	emit('update', {	'flags': simulator.get_flags(),
...
def stop(signum, frame):
	...
	logger.debug(f"Reason for stopping: {signal.Signals(signum).name} ({signum})")
	socketio.stop()
	os._exit(0)
...
def run(host='127.0.0.1', port=5000):
	...
	"""	
	logger.info(f"Starting websocket on {host}:{port}")
